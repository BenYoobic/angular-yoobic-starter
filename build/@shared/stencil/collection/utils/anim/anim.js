...
export function setAnimation(animationName, targets, options = {}) {
    if (Array.isArray(options.easing)) {
        options.easing = `cubicBezier(${options.easing.join(',')})`;
    }
...
export function backgroundFade(options = {}) {
    ...
    return {
        opacity: [options.open ? 0 : targetOpacity, options.open ? targetOpacity : 0],
        easing: 'linear',
...
const INFINITE_LEFT_SLIDE = 'infinite_left_slide';
export function infiniteLeftSlide(options = {}) {
    return {
        easing: 'linear',
        loop: true,
        left: [`${options.startPosition}px`, `${options.endPosition}px`],
        duration: options.duration
    };
}
const LEFT_SLIDE = 'left_slide';
export function leftSlide(options = {}) {
    return {
        easing: 'linear',
        width: [`${options.startWidth}px`, `${options.endWidth}px`],
        left: [`${options.startPosition}px`, `${options.endPosition}px`],
        duration: options.duration
    };
}
...
export function wooble(options = {}) {
    ...
    return Object.assign({ offset: 300, translateX: [{ value: '-15%', duration: totalDuration * 0.15, easing: 'easeInQuad' }, { value: '20%', duration: totalDuration * 0.15, easing: 'easeInQuad' }, { value: '-7%', duration: totalDuration * 0.15, easing: 'easeInQuad' }, { value: '5%', duration: totalDuration * 0.15, easing: 'easeInQuad' }, { value: '-3%', duration: totalDuration * 0.15, easing: 'easeInQuad' }, { value: '0%', duration: totalDuration * 0.15, easing: 'easeInQuad' }], rotateZ: [{ value: '-5deg', duration: totalDuration * 0.15, easing: 'easeInQuad' }, { value: '3deg', duration: totalDuration * 0.15, easing: 'easeInQuad' }, { value: '-3deg', duration: totalDuration * 0.15, easing: 'easeInQuad' }, { value: '2deg', duration: totalDuration * 0.15, easing: 'easeInQuad' }, { value: '-1deg', duration: totalDuration * 0.15, easing: 'easeInQuad' }, { value: '0', duration: totalDuration * 0.15, easing: 'easeInQuad' }] }, options);
}
const SHAKE = 'shake';
...
export function slideInStaggered(options = {}) {
    return Object.assign({ translateX: [-300, 0], opacity: [0, 1], delay: (el, i) => {
            if (el) {
                return i * 100;
            }
            return 0;
...
export function slideVertical(options = {}) {
    const duration = options.duration ? options.duration : 100;
    const distance = options.distance ? options.distance : 100;
    const easing = options.easing ? options.easing : 'linear';
    let animation = {
...
export function slideHorizontal(options = {}) {
    ...
    const easing = options.easing ? options.easing : 'linear';
    let animation = {
        translateX: [(options.open ? options.distance : start) * (options.left ? -1 : 1), (options.open ? start : options.distance) * (options.left ? -1 : 1)],
        easing: easing,
        duration
    };
    return animation;
...
const SIMPLE_SCALE = 'simple_scale';
export function simpleScale(options = {}) {
    let scaleMax = options.scaleMax ? options.scaleMax : 1.3;
    let scaleMin = options.scaleMin ? options.scaleMin : 1;
    return {
        scale: [options.open ? scaleMin : scaleMax, options.open ? scaleMax : scaleMin],
        easing: 'linear',
        duration: 200
    };
}
export const animations = {
    ...
    infiniteLeftSlide: INFINITE_LEFT_SLIDE,
...
export function animationFactory(animationName, options = {}) {
    switch (animationName) {
        ...
        case INFINITE_LEFT_SLIDE:
            return infiniteLeftSlide(options);
        case NEWSPAPER:
        ...
        case SIMPLE_SCALE:
            return simpleScale(options);
        case LEFT_SLIDE:
            return leftSlide(options);
        default:
            return {};
    }
}
